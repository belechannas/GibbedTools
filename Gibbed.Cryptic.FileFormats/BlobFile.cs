...
namespace Gibbed.Cryptic.FileFormats
{
    public class BlobFile
    {
        ...
        public string Type;
        public uint Flags;
        public MemoryStream ExtraFileInfo;
        ...
        public void Serialize(Stream output)
        {
            ...
            //output.WriteValueU32(1);  // flags value was previously hard-coded
            Console.WriteLine("Writing Flags value of {0}", this.Flags);
            ...
            //using (var data = new MemoryStream())
            //{
            //    data.WriteValueS32(0);  // hard-coded extra file info placeholder
            ...
            // pass through exactly what was read instead of hard-coded zero
            this.ExtraFileInfo.Position = 0;
            Console.WriteLine("Writing extra file info of length {0}", (uint) this.ExtraFileInfo.Length);
            output.WriteValueU32((uint)this.ExtraFileInfo.Length);
            output.WriteFromStream(this.ExtraFileInfo, (uint)this.ExtraFileInfo.Length);
        ...
        public void Deserialize(Stream input)
        {
            ...
            Console.WriteLine("Read Flags value of {0}", this.Flags);
            //if (flags != 1)
            //{
            ...
            this.ExtraFileInfo = input.ReadToMemoryStream(extraFileInfoSize);
            Console.WriteLine("Read extra file info of length: {0}", (uint)this.ExtraFileInfo.Length);
            //using (var data = input.ReadToMemoryStream(extraFileInfoSize))
            //{
            //    var count = data.ReadValueU32();  // was hard-coded as a single zero
            //    if (count != 0)
            //    {
